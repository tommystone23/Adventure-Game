...
void SpriteBatch::render_batch()
{
    ...
    for(size_t i = 0; i < _render_batches.size(); i++)
    {
        ...
        int num_indices = _render_batches[i].num_vertices * 1.5;
        int index_offset = _render_batches[i].offset * 1.5;
        glDrawElements(GL_TRIANGLES, num_indices, GL_UNSIGNED_INT, (void*)(index_offset * sizeof(GLuint)));
    }
...
void SpriteBatch::create_render_batches()
{
    ...
    vertices.resize(_glyph_pointers.size() * 4);

    std::vector<GLuint> indices;
    indices.resize(_glyph_pointers.size() * 6);
    ...
    _render_batches.emplace_back(offset, 4, _glyph_pointers[0]->texture);
    ...
    for(size_t cur_glyph = 1; cur_glyph < _glyph_pointers.size(); cur_glyph++)
    {
        ...
        {
            _render_batches.emplace_back(offset, 4, _glyph_pointers[cur_glyph]->texture);
        }
        else
        {
...
void SpriteBatch::init_indices(std::vector<GLuint> *indices)
{
    static GLuint base_index[] = {
        0, 1, 2,
        2, 3, 0
    };
